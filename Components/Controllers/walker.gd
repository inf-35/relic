...
func _ready():
	...
	nav_agent.path_desired_distance = 1 #increases precision of pathfinding (do this for all melee enemies)
...
func _process(delta):
	...
	match state:
		"loiter":
			if (GameDirector.player.entity.position - entity.position).length_squared() > 140000:
				return
			...
			if (GameDirector.player.entity.position - entity.position).length_squared() < 4000:
				state = "chase"
				return
			...
			if timer.time_left == 0:
				...
				while true:
					attempts += 1
					nav_agent.target_position = entity.position + Vector2(randf_range(-20,20),randf_range(-20,20))
